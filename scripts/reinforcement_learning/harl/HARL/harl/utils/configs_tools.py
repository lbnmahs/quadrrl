...
def get_defaults_yaml_args(algo, env):
    ...
    """
    if env != "isaaclab":
        raise NotImplementedError(f"Only 'isaaclab' environment is supported, got '{env}'")
    base_path = os.path.split(os.path.dirname(os.path.abspath(__file__)))[0]
    algo_cfg_path = os.path.join(base_path, "configs", "algos_cfgs", f"{algo}.yaml")
    # Note: Isaac Lab environments don't use env config YAML files from HARL
    # They use their own Hydra-based config system
    with open(algo_cfg_path, "r", encoding="utf-8") as file:
        algo_args = yaml.load(file, Loader=yaml.FullLoader)
    env_args = {}  # Isaac Lab envs provide config via env_args["config"]
